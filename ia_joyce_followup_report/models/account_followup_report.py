...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        """
        headers = [{},
                   {'name': _('EPOS Number'), 'style': 'text-align:center; white-space:nowrap;'},
                   ...
                   {'name': _('Purchase Order'), 'style': 'text-align:center; white-space:nowrap;'},
        ...
        if self.env.context.get('print_mode'):
            #headers = headers[:5] + headers[7:]  # Remove the 'Expected Date' and 'Excluded' columns  #ia change 8aug2019
            headers = headers[:5] + headers[9:]  # Remove the 'Expected Date' and 'Excluded' columns
        return headers
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                if self.env.context.get('print_mode'):
                    #columns = columns[:4] + columns[6:] #ia change 8aug2019
                    columns = columns[:4] + columns[8:]
                lines.append({
                    'id': aml.id,
            ...
            lines.append({
                ...
                'unfoldable': False,
                'level': 0,
                'columns': [{'name': v} for v in [''] * (3 if self.env.context.get('print_mode') else 7) + [total >= 0 and _('Total Due') or '', total_due]],
            })
            if total_issued > 0:
                ...
                lines.append({
                    ...
                    'unfoldable': False,
                    'level': 0,
                    'columns': [{'name': v} for v in [''] * (3 if self.env.context.get('print_mode') else 7) + [_('Total Overdue'), total_issued]],
                })
