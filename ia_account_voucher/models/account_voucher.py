...
class AccountVoucher(models.Model):
    ...
    @api.multi
    def reverse_voucher(self):
        self.env['account.move'].browse([self.move_id.id]).reverse_moves(self.date, self.journal_id or False)
        self.write({'state': 'cancel', 'move_id': False})

    @api.multi
    def cancel_voucher(self):
        for voucher in self:
            #self.mapped('move_id.line_ids.payment_id').cancel()
    ...
    def first_move_line_get(self, move_id, company_currency, current_currency):
        ...
        move_line = {
                ...
                'debit': debit,
                'credit': credit,
                'account_id': self.payment_journal_id.default_credit_account_id.id,
                'move_id': move_id,
    ...
    def _track_subtype(self, init_values):
        if 'state' in init_values:
            return 'ia_account_voucher.mt_voucher_state_change'
...
class AccountVoucherReverse(models.TransientModel):
    _name = "account.voucher.reverse"
    _description = "Reverse Document"

    @api.model
    def _default_date(self):
        vouchers = self.env[self._context['active_model']].browse(self._context['active_id'])
        return vouchers.account_date

    date = fields.Date(string='Reversal date', default=_default_date, required=True)


    def reverse_voucher(self):
        vouchers = self.env[self._context['active_model']].browse(self._context['active_id'])
        for voucher in vouchers:
            voucher.mapped('move_id.line_ids')._update_check()
            self.env['account.move'].browse([voucher.move_id.id]).reverse_moves(self.date, voucher.journal_id or False)
            voucher.write({'state': 'cancel', 'move_id': False, 'number': False})
